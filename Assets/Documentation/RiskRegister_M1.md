...
**Date:** M1 final cleanup  
**Schedule:** 10 working days foundation  
...
| R3 | ~~Config Content vs Resources drift~~ | — | — | **Closed** — single `Content/Resources/GameConfigRegistry` | Eng |
...
| R6 | Enemy/wave still scene-serialized — SO tables unused until M3 | High | Med | WaveTable/EnemyDefinition stubs ready; migrate WaveBossManager + enemy pooling together in M3 | Eng |
...
| R9 | Singleton coupling blocks global ability wiring | Med | Med | Prefer GameServices / GameplayEvents for new cast path | Eng |
...
| R11 | AddressableAssetsData not committed yet | Med | Med | Run **Initialize Addressables Groups** once; commit folder before heavy M2 VFX | Eng |

## Open actions before client “M1 paid/accepted”

1. Produce Development APK and complete [Android_BuildSmoke.md](Android_BuildSmoke.md) on one **physical** device (do not auto-pass).  
2. Client/design sign [NamingMap.md](NamingMap.md) approval table.  
3. Run Editor smoke from Bootstrap (see M1 final QA checklist).  
4. Generate and commit `Assets/AddressableAssetsData/` via Foundation menu.
